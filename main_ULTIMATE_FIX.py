"""
Harmonia Main - ULTIMATE FIX
Uses EXACT same safe wrapper from gemini_service_WORKING.py that WORKS!
"""
...
def _get_text_from_response(response) -> str:
    ...
    if hasattr(response, 'text'):
        try:
            ...
            # Index error or ValueError happens here - catch it and continue!
            pass
...
async def generate_response(request: ResponseGeneratorRequest):
    """
    Generate response - USES EXACT WORKING METHOD!
    """
    print(f"\n🤖 ═══════════════════════════════════════")
