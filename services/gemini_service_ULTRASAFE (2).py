...
class GeminiService:
    ...
    def _extract_text_safely(self, response):
        ...
        Handles empty responses, blocked content, etc.
        """
        ...
        if hasattr(response, 'text'):
            try:
                ...
                if text and len(text.strip()) > 0:
                    ...
                    print(f"   ⚠️ response.text is empty or whitespace")
                            ...
                            if hasattr(part, 'text'):
                                try:
                                    ...
                                    if text and len(text.strip()) > 0:
                                        ...
                                        print(f"   ⚠️ Part text is empty")
                                except Exception as e:
                                    print(f"   Method 2 failed: {e}")
        
        # Method 3: Check for prompt_feedback (blocked prompts)
        if hasattr(response, 'prompt_feedback'):
            feedback = response.prompt_feedback
            print(f"   Prompt feedback: {feedback}")
            if hasattr(feedback, 'block_reason'):
                raise Exception(f"Prompt blocked: {feedback.block_reason}")
        
        raise Exception("Could not extract text - response may be empty or blocked")
