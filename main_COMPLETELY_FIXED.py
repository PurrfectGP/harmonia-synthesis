"""
Harmonia Main - COMPLETELY FIXED
OLD SDK + GEMINI 3 PRO + SAFE WRAPPER
"""
...
async def generate_response(request: ResponseGeneratorRequest):
    """
    Generate response - OLD SDK + GEMINI 3 PRO + SAFE WRAPPER!
    """
    print(f"\n🤖 ═══════════════════════════════════════")
...
3. DEPTH: Show psychological complexity and mixed motivations
4. NATURALNESS: Conversational first-person, not formal
5. AMBIGUITY: Could reflect multiple personality traits
...
Return ONLY the response text, no preamble."""
        ...
        # TRY EACH MODEL WITH SAFE WRAPPER!
        for idx, model_name in enumerate(models_to_try, 1):
            try:
                ...
                # SAFE TEXT EXTRACTION (same as gemini_service_WORKING.py!)
                text = ""
